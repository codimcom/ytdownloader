...
def download_video(link: str, start: int, end: int):
    try:
        # Скачивание видео
        yt = YouTube(link)
        if start >= 3:
        ...
        print("успешно")
        """
        logging.info("starting cutting")
        print(start, end)
        
        with VideoFileClip('videos/output.mp4') as video:
            new = video.subclipped(start-start_1)
            new.write_videofile('videos/output_cut.mp4')
        os.remove('videos/output.mp4')
        os.rename("videos/output_cut.mp4", 'videos/output.mp4')
